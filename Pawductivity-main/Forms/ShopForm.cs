...
public class ShopForm : Form
{
    ...
    private readonly Action<PetChangeResult> _onBuy; // Callback after successful purchase
    private Label _lblCoins = null!;      // Label showing player's coins

    public ShopForm(GameManager gm, Action<PetChangeResult> onBuy)
    {
    ...
    private Panel BuildItemCard(ShopItem item, int y)
    {
        ...
        btnBuy.Click += (s, e) =>
        {
            ...
            if (change.Success)
            {
                ...
                _onBuy(change);        // trigger callback
